...
// Render strength as filled dots e.g. ●●●○○
...
/* ── Custom Pills block — shown under product name ── */
    ...
    <div style={{ display: "flex", flexWrap: "wrap", gap: 5, marginTop: 6 }}>
      ...
      {item.strength != null && (
        <span className="custom-pill" style={{ gap: 6 }}>
          <FlameIcon />
          Strength&nbsp;<StrengthDots value={item.strength} />
        </span>
      )}
      ...
      {item.sweetness != null && (
        <span className="custom-pill">
          <SugarIcon />
          {SWEETNESS_LABELS[item.sweetness] ?? `Level ${item.sweetness}`}
...
const CartPage = () => {
  ...
  // Compute totals — includes extra sweetness charge if sweetness > 3
  const enriched = cart.map((item) => {
    const basePrice  = Number(item.product?.basePrice ?? 0);
    const beanAdd    = Number(item.bean?.priceAdd ?? 0);
    const milkAdd    = Number(item.milk?.priceAdd ?? 0);
