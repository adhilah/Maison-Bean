...
const SkeletonRow = () => (
  <tr className="border-b border-[#c9a96e]/05 animate-pulse">
    {[80, 150, 90, 80, 100, 120, 110, 100].map((w, i) => (
      <td key={i} className="px-6 py-4">
        <div className="h-2.5 bg-[#ffffff06] rounded-full" style={{ width: w }} />
      </td>
    ))}
...
const StatusPill = ({ status }) => {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 px-2.5 py-1 text-[10px] tracking-[0.18em] uppercase font-medium border ${st.bg} ${st.border} ${st.text}`}
      style={{ fontFamily: "'Jost', sans-serif" }}
    >
      <span className="w-[5px] h-[5px] rounded-full flex-shrink-0" style={{ background: st.dot, boxShadow: `0 0 4px ${st.dot}99` }} />
      {st.label}
...
const StatusChanger = ({ order, onUpdate }) => {
  const allowed = NEXT_STATUSES[order.status] || ["Pending"];
  const next    = allowed.find((v) => v !== order.status);
  if (!next) return <span className="text-[#f5f0e8]/15 text-[10px] tracking-[0.2em] uppercase font-['Jost',sans-serif]">—</span>;
  ...
  return (
    <button
      onClick={() => onUpdate(order.id, next)}
      className="group inline-flex items-center gap-2 px-3 py-1.5 text-[10px] tracking-[0.18em] uppercase transition-all duration-200 border border-[#c9a96e]/15 text-[#c9a96e]/40 hover:border-[#c9a96e]/40 hover:text-[#c9a96e]/80 hover:bg-[#c9a96e]/04 font-['Jost',sans-serif]"
      title={`Move to ${nextSt.label}`}
    >
      <span className="w-[5px] h-[5px] rounded-full flex-shrink-0" style={{ background: nextSt.dot, opacity: 0.5 }} />
      <span>{nextSt.label}</span>
      <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="opacity-50 group-hover:translate-x-0.5 transition-transform duration-150">
        <line x1="5" y1="12" x2="19" y2="12" /><polyline points="12 5 19 12 12 19" />
      </svg>
  ...
  return (
    <>
      <style>{`
        ...
        .om-in  { animation: fadeUp 0.45s ease forwards; }
        .om-row { animation: fadeUp 0.3s ease both; }
        .om-row:hover td { background: rgba(201,169,110,0.018); }
      ...
      <div className="min-h-screen bg-[#0d0a05] font-['Jost',sans-serif] px-4 sm:px-6 lg:px-14 py-10 lg:py-14">
        ...
        <div className="relative z-10 max-w-screen-xl mx-auto">
          ...
          {/* ── Page Header ── */}
          <div className="mb-10 om-in">

            {/* Top row: breadcrumb + dashboard */}
            <div className="flex items-start justify-between gap-4 mb-3">
              <div>
                <p className="text-[#c9a96e] text-[10px] tracking-[0.55em] uppercase mb-3 opacity-65">
                  Admin · Orders
                </p>
                <h1 className="font-['Cormorant_Garamond',serif] font-light text-[#f5f0e8] leading-none" style={{ fontSize: "clamp(2rem,4vw,3rem)" }}>
                  Order <span className="italic text-[#c9a96e]">Records</span>
                </h1>
              </div>

              <Link
                to="/admin/dashboard"
                className="group flex items-center gap-3 text-[#c9a96e]/55 hover:text-[#c9a96e] transition-all uppercase tracking-[0.4em] text-[10px] flex-shrink-0 mt-1"
              >
          ...
          {/* ── Panel ── */}
          <div className="border border-[#c9a96e]/12 om-in" style={{ animationDelay: "80ms" }}>

            {/* Panel header: icon + title + search */}
            <div className="flex items-center justify-between gap-4 px-6 py-5 bg-[#110d07] border-b border-[#c9a96e]/10">
              ...
              {/* Search */}
              <div className="flex items-center gap-2 border-b border-[#c9a96e]/15 pb-1 hover:border-[#c9a96e]/30 transition-colors">
                ...
                <input
                  ...
                  onChange={(e) => setSearch(e.target.value)}
                  placeholder="Search ID or email…"
                  className="bg-transparent text-[#f5f0e8]/70 text-[12px] font-light placeholder:text-[#f5f0e8]/18 focus:outline-none w-52"
                />
              </div>
            </div>
            ...
            <div className="flex divide-x divide-[#c9a96e]/08 border-b border-[#c9a96e]/08">
              {[
                { label: "Total",     value: orders.length,                                                                                         color: "text-[#f5f0e8]/60" },
                { label: "Pending",   value: counts["Pending"]   || 0,                                                                              color: "text-[#f5a623]"    },
                { label: "Active",    value: (counts["Processing"] || 0) + (counts["Shipping"] || 0) + (counts["OutForDelivery"] || 0),             color: "text-[#3b82f6]"    },
                { label: "Delivered", value: counts["Delivered"] || 0,                                                                              color: "text-[#4ade80]"    },
                { label: "Cancelled", value: counts["Cancelled"] || 0,                                                                              color: "text-[#f87171]/70" },
                ...
                <div key={label} className="flex-1 py-3 px-5 text-center">
                  ...
                  <p className="text-[9px] tracking-[0.3em] uppercase text-[#f5f0e8]/22 mt-0.5">{label}</p>
                </div>
              ))}
            </div>

            {/* Filter tabs */}
            <div className="flex overflow-x-auto border-b border-[#c9a96e]/08 bg-[#0d0a05]">
              {tabs.map((t) => (
                <button
                  key={t.value}
                  onClick={() => setFilter(t.value)}
                  className={`
                    flex items-center gap-2 px-4 py-3 text-[9px] tracking-[0.25em] uppercase
                    whitespace-nowrap border-b-[1.5px] transition-all duration-150
                    ${filter === t.value
                      ? "border-[#c9a96e] text-[#c9a96e]"
                      : "border-transparent text-[#f5f0e8]/22 hover:text-[#f5f0e8]/50"
                    }
                  `}
                >
                  {t.dot && (
                    <span className="w-[5px] h-[5px] rounded-full flex-shrink-0" style={{ background: t.dot, opacity: filter === t.value ? 0.9 : 0.35 }} />
                  )}
                  {t.label}
                  <span className={`text-[8px] px-1.5 py-0.5 ${filter === t.value ? "bg-[#c9a96e]/15 text-[#c9a96e]/80" : "bg-[#ffffff]/04 text-[#f5f0e8]/18"}`}>
                    {t.count}
                  </span>
                </button>
              ))}
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead>
                  <tr className="border-b border-[#c9a96e]/06">
                    {["Order", "Customer", "Date", "Total", "Payment", "Status", "Next Step"].map((h) => (
                      <th
                        key={h}
                        className="px-6 py-3 text-left text-[8.5px] tracking-[0.45em] uppercase text-[#c9a96e]/22 font-normal whitespace-nowrap font-['Jost',sans-serif]"
                      >
                        {h}
                      </th>
                    ))}
                  ...
                  {loading ? (
                    ...
                    <tr>
                      <td colSpan={8} className="py-28 text-center">
                        <p className="font-['Cormorant_Garamond',serif] italic text-[#f5f0e8]/15 text-[1.2rem]">
                          No orders found
                        </p>
                      </td>
                    </tr>
                    ...
                    filtered.map((order, idx) => {
                      ...
                      return (
                        ...
                        >
                          {/* Order ID */}
                          <td className="px-6 py-4 whitespace-nowrap">
                            <span className="font-['Cormorant_Garamond',serif] text-[1rem] text-[#c9a96e]/70 font-light tracking-wider">
                              #{String(order.id).slice(-8).toUpperCase()}
                            </span>
                          </td>
                          ...
                          {/* Customer */}
                          <td className="px-6 py-4 max-w-[180px]">
                            <p className="text-[#f5f0e8]/45 text-[11px] truncate tracking-wide">
                              {order.userEmail || "Guest"}
                            </p>
                          </td>
                          ...
                          {/* Payment */}
                          <td className="px-6 py-4 whitespace-nowrap">
                            <p className="text-[#f5f0e8]/28 text-[11px] tracking-wide">{payLabel}</p>
                          </td>
