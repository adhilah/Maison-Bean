...
const ProductRow = ({ product, onEdit, onDelete, index }) => {
  ...
  const isDimmed = isBlocked || isOutOfStock;
      ...
      <div className="w-12 h-12 border border-[#c9a96e]/15 overflow-hidden bg-[#110d07] flex-shrink-0">
        <img
          ...
          className={`w-full h-full object-cover transition-all duration-300 ${
            isDimmed  ? "opacity-25 grayscale" : "opacity-80 group-hover:opacity-100"
          }`}
        />
      </div>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 flex-wrap">
          <p className={`font-['Cormorant_Garamond',serif] text-[1.05rem] truncate ${
            isDimmed ? "text-[#f5f0e8]/40"  : "text-[#f5f0e8]/85" }`}>
            {product.name}
          </p>
          {isOutOfStock && (
  <span className="
  ...
  ">
    OUT OF STOCK
        ...
        <p className={`text-[11px] mt-1 font-['Jost',sans-serif] ${
          isDimmed  ? "text-[#f5f0e8]/18" : "text-[#f5f0e8]/28"
        }`}>
      ...
      <div className="min-w-[90px] text-right flex-shrink-0">
        <span className={`font-['Cormorant_Garamond',serif] text-[1rem] ${
          isDimmed  ? "text-[#f5f0e8]/22" : "text-[#c9a96e]/80"
        }`}>
...
export default function ProductList() {
  ...
  return (
    <>
      ...
      {confirm.open && (
  <ConfirmDialog
    title={`Delete "${confirm.product?.name}"?`}
