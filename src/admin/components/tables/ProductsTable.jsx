...
const ProductRow = ({ product, onEdit, onDelete, onToggle, setConfirm, index }) => {
  const isBlocked = product.isBlocked;

  return (
    <div
      className={`group flex items-center gap-5 px-6 py-4 border-b border-[#c9a96e]/08 transition-all duration-300 bg-[#0d0a05] hover:bg-[#110d07] ${
    ...
    >
      ...
      {/* Product Image */}
      <div className="flex-shrink-0">
        <div className="w-12 h-12 border border-[#c9a96e]/15 bg-[#110d07] overflow-hidden">
          <img
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 flex-wrap">
          <p className="font-['Cormorant_Garamond',serif] text-[1.05rem] font-light truncate leading-tight text-[#f5f0e8]/85">
            {product.name}
          </p>
          {isBlocked && (
            <span className="text-[9px] tracking-[0.3em] uppercase font-['Jost',sans-serif] text-[#f87171]/70 flex-shrink-0">
              Blocked
            </span>
          )}
        </div>
        <p className="text-[11px] font-['Jost',sans-serif] font-light mt-0.5 truncate text-[#f5f0e8]/28">
          {product.category || "—"} · ID: {product.id}
        </p>
      </div>
  ...
  const fetchProducts = async () => {
    try {
      ...
      const normalized = (data || []).map((product) => ({
        ...product,
        isBlocked: product.isBlocked ?? product.blocked ?? false,
      }));
      setProducts(normalized);
    } catch (error) {
      console.error(error);
          ...
          <div className="bg-[#0d0a05] border border-[#c9a96e]/12 flex flex-col pl-in">
            ...
            <div className="flex divide-x divide-[#c9a96e]/08 border-b border-[#c9a96e]/08">
              {[
                ...
                { label: "Blocked",  value: blockedCount,             color: "text-[#f87171]/70" },
              ].map(({ label, value, color }) => (
                <div key={label} className="flex-1 py-3 px-5 text-center">
                  <p className={`font-['Cormorant_Garamond',serif] text-[1.4rem] font-light ${color || "text-[#f5f0e8]/60"}`}>
                    {value}
